...
class Attention_Module(nn.Module):
    def __init__(self, args, embed, num_symbols, embedding_size, use_pretrain=True, finetune=True, dropout_rate=0.3):
        ...
        self.support_query_attention_module = Attention(hidden_size=self.embedding_size, num_heads=1) #
    ...
    def SPD_attention(self, entity_left, entity_right, rel_emb_forward, rel_emb_backward,
        ...
        #
        s_V_head=s_V_head.unsqueeze(0).repeat(enhanced_head.size(0),1,1)
        head_final=self.support_query_attention_module(q=enhanced_head,k=s_V_head,v=s_V_head)
        head_final=torch.relu(self.Linear_tail(head_final)+self.Linear_head(entity_left))
        head_final=self.layer_norm(head_final+entity_left+enhanced_head)

        #
        s_V_tail=s_V_tail.unsqueeze(0).repeat(enhanced_head.size(0),1,1)
        tail_final=self.support_query_attention_module(q=enhanced_tail,k=s_V_tail,v=s_V_tail)
        tail_final=torch.relu(self.Linear_tail(tail_final)+self.Linear_head(entity_right))
        tail_final=self.layer_norm(tail_final+entity_right+enhanced_tail)

        # computing entity-pair representation
        enhanced_pair = torch.cat([head_final, tail_final], dim=-1)
        ent_pair_rep = self.bi_MLP(enhanced_pair)
        return ent_pair_rep


    def forward(self, entity_pairs, entity_meta,support_meta):
        ...
        s_entity_left_connections,_, s_entity_right_connections,_=support_meta
        ...
        s_relations_left=s_entity_left_connections[:, :, 0].view(-1)
        s_entities_left=s_entity_left_connections[:, :, 1].view(-1)
        s_rel_embeds_left=self.dropout(self.symbol_emb(s_relations_left))
        s_ent_embeds_left=self.dropout(self.symbol_emb(s_entities_left))

        s_relations_right=s_entity_right_connections[:, :, 0].view(-1)
        s_entities_right=s_entity_right_connections[:, :, 1].view(-1)
        s_rel_embeds_right=self.dropout(self.symbol_emb(s_relations_right))
        s_ent_embeds_right=self.dropout(self.symbol_emb(s_entities_right))
        s_V_head=torch.relu(self.gate_w(torch.cat([s_rel_embeds_left, s_ent_embeds_left], dim=-1)))
        s_V_tail=torch.relu(self.gate_w(torch.cat([s_rel_embeds_right, s_ent_embeds_right], dim=-1)))
