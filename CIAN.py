...
class CIAN(nn.Module):
    ...
    def forward(self, support, support_meta,
        ...
        if is_train:
            ...
            support_rep = self.entity_encoder(support, support_meta,support_meta)
            query_rep = self.entity_encoder(query, query_meta,support_meta)
            false_rep = self.entity_encoder(false, false_meta,support_meta)
            ...
            negative_score = torch.sum(false_rep * support_false, dim=1).view(positive_score.shape[0],-1)
            negative_score = (F.softmax(negative_score,dim=1).detach()*negative_score).sum(dim=1)
